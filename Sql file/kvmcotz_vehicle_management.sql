...
--
-- Stand-in structure for view `exipenditure_report`
-- (See below for the actual view)
--
CREATE TABLE `exipenditure_report` (
`first_name` varchar(255)
,`last_name` varchar(255)
,`email` varchar(255)
,`username` varchar(255)
,`expenditure_type_name` varchar(100)
,`exp_id` int(11)
,`expenditure_type` int(11)
,`expenditure_descrption` text
,`expenditure_amount` bigint(20)
,`expenditure_date` timestamp
,`user_expenditure` int(11)
);
...
--
-- Structure for view `exipenditure_report`
--
DROP TABLE IF EXISTS `exipenditure_report`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `exipenditure_report`  AS  select `user`.`first_name` AS `first_name`,`user`.`last_name` AS `last_name`,`user`.`email` AS `email`,`user`.`username` AS `username`,`tbl_expenditure_type`.`expenditure_type_name` AS `expenditure_type_name`,`tbl_expenditure`.`exp_id` AS `exp_id`,`tbl_expenditure`.`expenditure_type` AS `expenditure_type`,`tbl_expenditure`.`expenditure_descrption` AS `expenditure_descrption`,`tbl_expenditure`.`expenditure_amount` AS `expenditure_amount`,`tbl_expenditure`.`expenditure_date` AS `expenditure_date`,`tbl_expenditure`.`user_expenditure` AS `user_expenditure` from ((`user` join `tbl_expenditure_type`) join `tbl_expenditure`) where `tbl_expenditure`.`expenditure_type` = `tbl_expenditure_type`.`expenditure_type_id` and `tbl_expenditure`.`user_expenditure` = `user`.`user_id` ;
